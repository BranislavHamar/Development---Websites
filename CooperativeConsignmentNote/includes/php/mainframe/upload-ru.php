...
////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        ...
        function ReadUrl($UrlParam,$Decode)
          ...
          //global $DB_Connection;
            ...
            If (ReadUrl("SenderTown","") == "" )                                                                              { $ValidationMessage .= "<tr><td>SenderTown:: </td><td><input type='text' name='SenderTown'></td><td><i>Town</i></td></tr>"; $stop=1;}
            If (ReadUrl("SenderPostcode","") == "" )                                                                          { $ValidationMessage .= "<tr><td>SenderPostcode:: </td><td><input type='text' name='SenderPostcode'></td><td><i>Postcode</i></td></tr>"; $stop=1;}
            ...
            If (strlen(ReadUrl("Service","")) < 3)                                                                            { $ValidationMessage .= "<tr><td>Service :: </td><td><input type='text' name='Service'></td><td><i>Service Code (eg. 48N)</i></td></tr>"; $stop=1;}
            If (strlen(ReadUrl("Division","")) < 1)                                                                           { $ValidationMessage .= "<tr><td>Division :: </td><td><input type='text' name='Division'></td><td><i>Division code (eg. D)</i></td></tr>"; $stop=1;}
        ...
        If (ReadUrl("InsuranceValue","Y") <> "") {
             $get_ShipmentValue = ReadUrl("InsuranceValue","Y");
        } else {
             $get_ShipmentValue = "";
        }
        ...
        If (ReadUrl("CashOnDeliveryValue","Y") <> "") {
             $get_DeliveryInstruction = "COD ".ReadUrl("CashOnDeliveryValue","")." ".ReadUrl("CashOnDeliveryCurrency","") ;
        } else {
             $get_DeliveryInstruction = "";
        }
        
        //Service name taken from DB based on Service Code and Service Division


 

        $sql  =   "SELECT a.Name
                   FROM cc_".$cfg_Sender_Country_Code."_cfg_services AS a
                   WHERE a.div = '".ReadUrl("Division","Y")."' AND a.Code = '".ReadUrl("Service","Y")."' ";

        $sql_select = mysqli_query($DB_Connection,$sql);  

          if (mysqli_num_rows($sql_select) > 0) {
            $row = mysqli_fetch_array($sql_select);
                    $get_Service = $row['Name'];
          } else {
                //$get_Service = "N/A";
                echo $sql;
                exit;
         ...
         $sql  =   "
                     ...
                     SenderPostcode            = '".ReadUrl("SenderPostcode","Y")."',

                     SenderTown                = '".strtoupper(ReadUrl("SenderTown","Y"))."',
                     ...
                     InsuranceValue            = '".ReadUrl("InsuranceValue","Y")."',

                     InsuranceCurrency         = '".ReadUrl("InsuranceCurrency","Y")."',

                     CashCollectionValue       = '".ReadUrl("CashCollectionValue","Y")."',

                     CashCollectionCurrency    = '".ReadUrl("CashCollectionCurrency","Y")."',

                     CashOnDeliveryValue       = '".ReadUrl("CashOnDeliveryValue","Y")."',
 		...
 		// **************************************************
            ...
            function send_email($to='', $from='', $subject='', $html_content='', $text_content='', $headers='', $bounce='') { 
              ...
              //return mail($to, $subject, $body, $headers);
                    ...
                    $messagehtml .= "<br>";
                    $messagehtml .= $cfg_csfl_message[8]."<a href='".$cfg_csfl_message[9]."'>".$cfg_csfl_message[9]."</a>";
                    $messagehtml .= "<br>";
                    $messagehtml .= $cfg_csfl_message[10]."<a href='".$cfg_csfl_message[11]."'>".$cfg_csfl_message[11]."</a>";
                    $messagehtml .= "<br>";
                    $messagehtml .= "<br>";
                    $messagehtml .= $cfg_csfl_message[12];
                    $messagehtml .= "<br>";
                    $messagehtml .= $cfg_csfl_message[13].$cfg_csfl_email_bounce;
                    $messagehtml .= "<br>";
                    $messagehtml .= $cfg_csfl_message[14];
                    $messagehtml .= "<br>";
                    $messagehtml .= $cfg_csfl_message[15];
                    $messagehtml .= "<br>";
                    $messagehtml .= $cfg_csfl_message[16]; 
                    $messagehtml .= "<br>";
                    $messagehtml .= "<a href='".$cfg_csfl_message[17]."'>".$cfg_csfl_message[17]."</a>";
                    $messagehtml .= "<br>";
                    $messagehtml .= $cfg_csfl_message[18];                   
                    ...
                    $messagetext .= "\r\n";
                    $messagetext .= $cfg_csfl_message[8].$cfg_csfl_message[9];
                    $messagetext .= "\r\n";
                    $messagetext .= $cfg_csfl_message[10].$cfg_csfl_message[11];
                    $messagetext .= "\r\n";
                    $messagetext .= "\r\n";
                    $messagetext .= $cfg_csfl_message[12];
                    $messagetext .= "\r\n";
                    $messagetext .= $cfg_csfl_message[13].$cfg_csfl_email_bounce;
                    $messagetext .= "\r\n";
                    $messagetext .= $cfg_csfl_message[14];
                    $messagetext .= "\r\n";
                    $messagetext .= $cfg_csfl_message[15];
                    $messagetext .= "\r\n";
                    $messagetext .= $cfg_csfl_message[16];
                    $messagetext .= "\r\n";
                    $messagetext .= $cfg_csfl_message[17];
                    $messagetext .= "\r\n";
                    $messagetext .= $cfg_csfl_message[18];
                    
                    if (mb_detect_encoding($cfg_csfl_subject,"UTF-8")) {
                      $cfg_csfl_subject = "=?UTF-8?B?".base64_encode($cfg_csfl_subject)."?=";              
                    }
