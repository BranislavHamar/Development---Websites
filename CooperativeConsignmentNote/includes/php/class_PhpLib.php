 ...
 *                    Kristian Koehntopp
 *
 * $god: template.inc,v 1.12 2002/07/11 22:29:51 richardarcher Exp $
 *
 */
 ...
 *
 *
 * Changes in functionality which i beyond bug fixes:
 *
  ...
  */
  function parse($target, $varname, $append = false) {
    if (!is_array($varname)) {
      ...
      reset($varname);
      while(list($go, $v) = each($varname)) {
        if ($this->debug & 4) {
          echo "<p><b>parse:</b> (with array) target = $target, i = $go, varname = $v, append = $append</p>\n";
        }
        $str = $this->subst($v);
        if ($append) {
          $this->set_var($target, $this->get_var($target) . $str);
